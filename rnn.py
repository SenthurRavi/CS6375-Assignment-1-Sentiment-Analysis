    ...
    while not stopping_condition and epoch < args.epochs:
        ...
        for minibatch_index in tqdm(range(0, len(train_data), 32)):
            ...
            for i in range(32):
                ...
                input_tensor = torch.tensor(np.array(input_vectors), dtype=torch.float32).unsqueeze(1).to(device)

                output = model(input_tensor)
                predicted_label = torch.argmax(output)

                batch_loss += model.compute_loss(output, torch.tensor([label], device=device))
